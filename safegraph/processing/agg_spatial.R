...
filenames <- filenames[-which(filenames %in% c("GU.RDS","MP.RDS","PR.RDS","VI.RDS","AL.RDS","CT.RDS","AK.RDS","AR.RDS","AZ.RDS"))]
...
for (i in 1:length(filenames)){
   ...
   df<- readRDS(paste("1_res/", filenames[i], sep = "")) %>%
    ...
    left_join(countypop, by= c("statefips"="statefips","countyfips"="countyfips")) %>%
